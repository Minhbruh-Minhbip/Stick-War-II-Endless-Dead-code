package com.brockw.stickwar.engine.units
{
   ...
   public class Skelator extends Unit
   {
      ...
      override public function init(param1:StickWar) : void
      {
         ...
         this.fistAttackSpell = new SpellCooldown(param1.xml.xml.Chaos.Units.skelator.fist.effect,param1.xml.xml.Chaos.Units.skelator.fist.cooldown + 3,param1.xml.xml.Chaos.Units.skelator.fist.mana);
      ...
      override public function update(param1:StickWar) : void
      {
         ...
         if(!isDieing)
         {
            ...
            else if(this.isFistAttacking)
            {
               ...
               if(_mc.mc.currentFrame == 27)
               {
                  this.clusterLad = team.game.unitFactory.getUnit(Unit.U_DEAD);
                  team.spawn(this.clusterLad,team.game);
                  this.clusterLad.px = this.px + 100;
                  this.clusterLad.py = this.py + 50;
                  this.clusterLad.deadType = "Light";
                  Dead(this.clusterLad).light();
                  this.clusterLad.stun(60);
                  this.directionAtBurrow = getDirection();
                  this.clusterLad.forceFaceDirection(-1);
                  team.game.projectileManager.initMound(this.clusterLad.px,this.clusterLad.py,0,team,-this.directionAtBurrow);
                  this.clusterLad = team.game.unitFactory.getUnit(Unit.U_DEAD);
                  team.spawn(this.clusterLad,team.game);
                  this.clusterLad.px = this.px - 100;
                  this.clusterLad.py = this.py + 50;
